        ...
        while (move_map) {
            ...
            if (is_promotion(row, col, to_row, to_col)) {
                for (auto p : {QUEEN, ROOK, BISHOP, KNIGHT}) {
                    moves.emplace_back(from_square, to_square, p, values[p] - values[PAWN]);
                }
            } else {
                int capture = board[to_row][to_col];
                int priority = capture ? values[get_piece_type(capture)] - values[get_piece_type(piece)] : 0;
                
                if (std::abs(col - to_col) > 1 && get_piece_type(piece) == KING) {
                    // castle is often a good move
                    priority += 3;
                }
                if (half_moves < 30 && to_row >= 2 && to_col >= 2 && to_row <= 5 && to_col <= 5) {
                    priority += 1;
                }
