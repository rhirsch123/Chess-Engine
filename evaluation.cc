...
namespace Evaluation {
    ...
    static uint64_t get_attackers(Position& position, int square, int color, uint64_t blockers = 0ULL) {
        ...
        if (!blockers) {
            // default to not including friendly pieces in blockers - lets it consider batteries and discoveries
            blockers = position.piece_maps[KING][WHITE] | position.piece_maps[KING][BLACK] |
                position.piece_maps[QUEEN][!color] | position.piece_maps[ROOK][!color] |
                position.piece_maps[BISHOP][!color] | position.piece_maps[KNIGHT][!color] |
    ...
    static int evaluate_rooks(Position& position, Engine& engine, int color, uint64_t opp_pawn_attacks) {
        ...
        while (rooks) {
            ...
            //mobility += num_set_bits(attacked_squares & get_column_mask(col));
        ...
        if (major_material > (Position::values[ROOK] * 4)) { // opening, middlegame
            ...
            for (int row = 2; row <= 5; row++) {
                for (int col = 2; col <= 5; col++) {
                    ...
                    int white_attacks = num_set_bits(get_attackers(position, square, WHITE));
                    int black_attacks = num_set_bits(get_attackers(position, square, BLACK));
