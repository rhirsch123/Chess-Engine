...
namespace Polyglot {
    ...
    const uint64_t * piece_table = key_table;
    const uint64_t * castle_table = piece_table + 12 * 64;
    const uint64_t * en_passant_table = castle_table + 4;
    ...
    uint64_t hash(const Position& position) {
        ...
        while (pieces) {
            ...
            int piece = position.board[row][col];
        ...
        if (position.can_castle[1]) hash ^= castle_table[0];
        if (position.can_castle[0]) hash ^= castle_table[1];
    ...
    template<typename T> T read_big_endian(std::ifstream& book) {
        T result = 0;
        for (size_t i = 0; i < sizeof(T); i++) {
            result = T((result << 8) | book.get());
        }
        return result;
    }

    polyglot_entry read_entry(std::ifstream& book) {
        polyglot_entry e;
        e.key   = read_big_endian<uint64_t>(book);
        e.move  = read_big_endian<uint16_t>(book);
        e.count = read_big_endian<uint16_t>(book);
        e.learn = read_big_endian<uint32_t>(book);
        return e;
    }

    // binary search
    size_t find_first(uint64_t key, std::ifstream& book) {
        book.seekg(0, std::ios::end);

        size_t low = 0, mid, high = (size_t) book.tellg() / sizeof(polyglot_entry) - 1;
        polyglot_entry entry;

        while (low < high && book.good()) {
            mid = (low + high) / 2;

            book.seekg(mid * sizeof(polyglot_entry), std::ios_base::beg);
            entry = read_entry(book);
    ...
    Move get_book_move(const Position& position, std::string file) {
        ...
        book.seekg(find_first(key, book) * sizeof(polyglot_entry), std::ios_base::beg);
        entry = read_entry(book);
        while (entry.key == key && book.good()) {
            sum += entry.count;

            // choose randomly weighted on how common the move is
            if (sum && random_U64() % sum < entry.count) {
                move = entry.move;
            }
        ...
        // polyglot move: 6 bits to_square, 6 bits from_square, 4 bits promotion
        // castles are king takes rook
        uint8_t from = (move >> 6) & 0x3F;
        int from_row = 7 - (from / 8);
        int from_col = from % 8;
        ...
        int to_col = to % 8;
        // change castle
        if (Position::get_piece_type(position.board[from_row][from_col]) == KING) {
            int dist = std::abs(to_col - from_col);
            if (dist == 4) { // queenside
                to_col = 2;
            } else if (dist == 3) { // kingside
                to_col = 6;
            }
        }
        to = to_row * 8 + to_col;

        uint8_t promotion = (move >> 12) & 0xF;
